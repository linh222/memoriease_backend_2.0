...
def rag_retriever(question, size, embedding_model, tokenizer, device):
    ...
    # embed the query
    tokenized_query = tokenizer([processed_query], padding=True, truncation=True, return_tensors='pt')
...
def RAG(question, embedding_model, tokenizer):
    # retrieve all data
    relevant_document = rag_retriever(question, 50, embedding_model, tokenizer)
    retrieved_result = []
...
def rag_question_answering(query, previous_chat, embedding_model, tokenizer, device):
    ...
    textual_answer, retrieved_result = RAG(question, embedding_model, tokenizer, device)
