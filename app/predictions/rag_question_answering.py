...
def RAG(question):
    # retrieve all data
    context_query_return, question_to_ask_return, question_to_confirm_return = process_question(question)
    logging.info(f"RAG: Query to retrieve: ", context_query_return)
    relevant_document = rag_retriever(context_query_return, 50)
    # Create prompt
    prompt = create_prompt(question_to_ask_return, relevant_document)
...
def rag_question_answering(query, blip2_embed_model, blip2_txt_processor,
